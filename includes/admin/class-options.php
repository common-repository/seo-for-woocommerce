...
use RankMath_Woocommerce\Replace_Vars;
...
class Options {
	...
	function register_option_page() {
		...
		$cmb = new_cmb2_box( [
			...
			'option_key'   => $this->key,
			'parent_slug'  => Helper::is_addon_active( 'schema-markup' ) ? 'rank-math-schema' : 'rank-math-woocommerce',
			'cmb_styles'   => false,
		...
		foreach ( $tabs as $id => $tab ) {
			...
			$cmb->add_field( [
				'name' => esc_html__( 'Panel', 'seo-for-woocommerce' ),
				'id'   => 'setting-panel-' . $id,
				'type' => 'tab_open',
			] );
	...
	public function reset_options() {
		if ( ! check_admin_referer( 'rank-math-woocommerce-reset-options' ) || ! current_user_can( 'manage_options' ) ) {
			return false;
		}
	...
	public function enqueue() {
		...
		wp_enqueue_style( 'rank-math-options', rank_math_woocommerce()->plugin_url() . 'assets/admin/css/option-panel.css', [ 'rank-math-common', 'rank-math-cmb2' ], rank_math_woocommerce()->version );
		wp_enqueue_script( 'rank-math-options', rank_math_woocommerce()->plugin_url() . 'assets/admin/js/option-panel.js', [ 'underscore', 'rank-math-common' ], rank_math_woocommerce()->version, true );

		// Add thank you.
		Helper::add_json( 'indexUrl', rank_math_woocommerce()->plugin_url() . 'assets/admin/js/search-index/' );
	...
	public function display( $machine ) {
		$cmb = $machine->cmb;
		?>
		<div class="wrap rank-math-wrap rank-math-wrap-settings <?php echo count( Helper::get_active_modules() ) > 1 ? 'addon-active' : ''; ?>>
			...
			<form class="cmb-form" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" method="POST" id="<?php echo $cmb->cmb_id; ?>" enctype="multipart/form-data" encoding="multipart/form-data">
				...
				<footer class="form-footer rank-math-ui settings-footer wp-clearfix">
					<?php wp_nonce_field( 'rank-math-woocommerce-reset-options' ); ?>
	...
	private function locate_file( $id, $tab ) {
		...
		$file = isset( $tab['file'] ) && ! empty( $tab['file'] ) ? $tab['file'] : rank_math_woocommerce()->includes_dir() . "settings/{$this->folder}/{$id}.php";
