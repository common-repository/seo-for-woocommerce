...
class CMB2 {
	...
	public static function pre_init( $cmb ) {
		foreach ( $cmb->prop( 'fields' ) as $id => $field_args ) {
			...
			if ( in_array( $type, [ 'meta_tab_container_open', 'tab_container_open', 'tab_container_close', 'tab_open', 'tab_close', 'raw' ] ) ) {
				$field->args['save_field']    = false;
				$field->args['render_row_cb'] = [ '\RankMath_Woocommerce\CMB2', "render_{$type}" ];
			}
			if ( 'notice' === $type ) {
				$field->args['save_field'] = false;
			}
	...
	public static function render_tab_container_open( $field_args, $field ) {
		...
		?>
		<?php if ( Helper::is_addon_active( 'redirections' ) ) { ?>
