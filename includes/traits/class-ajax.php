 ...
 * @since      1.0.0
 * @package    RankMath
 * @subpackage RankMath_Woocommerce\Traits
 * @author     Rank Math <support@rankmath.com>
 */

namespace RankMath_Woocommerce\Traits;
...
trait Ajax {
	...
	protected function ajax( $tag, $function_to_add, $priority = 10 ) {
		\add_action( 'wp_ajax_rank_math_woocommerce_' . $tag, [ $this, $function_to_add ], $priority );
	}
	...
	public function verify_nonce( $action ) {
		if ( ! isset( $_REQUEST['security'] ) || ! \wp_verify_nonce( $_REQUEST['security'], $action ) ) {
			$this->error( __( 'Error: Nonce verification failed', 'seo-for-woocommerce' ) );
		}
	}
